...
// b2b_item_groups ve b2b_group_members tablolarını kullanan akıllı arama helper'ı
    ...
    try {
        ...
        return {
            ...
            search_metadata: {
                ...
                response_time_ms: 0,
                cache_hit: false,
                note: 'Grup tablosunda eşleşme bulunamadı, LIKE araması kullanıldı'
            }
        };
    } catch (error) {
...
async function smartSearch(query, customerCode = null, limit = 50) {
    ...
    try {
        ...
        // KURAL: 4 veya daha az karakter -> OEM öncelikli arama, aksi halde direkt grup araması
        if (normalizedQuery.length <= 4) {
            console.log(`🔍 Kısa kod (${normalizedQuery}): OEM öncelikli arama`);

            // Önce LOGOGO3'te OEM kodunda tam eşleşme ara
            ...
            const oemResult = await oemRequest.query(`
                SELECT TOP 20
                    ...
                    STGRPCODE
                FROM LG_013_ITEMS
                WHERE UPPER(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(ISNULL(PRODUCERCODE, ''), ' ', ''), '-', ''), '.', ''), '/', ''), '\\', ''), '_', '')) = @norm
            ...
            if (oemResult.recordset.length > 0) {
                console.log(`✅ ${oemResult.recordset.length} OEM eşleşmesi bulundu`);

                // Bu malzemelerin gruplarını B2B_TRADE_PRO'da bul
                ...
                console.log('⚠️ OEM için grup bulunamadı, karakter aramasına geçiliyor');
            } else {
                console.log('⚠️ OEM eşleşmesi bulunamadı, karakter aramasına geçiliyor');
            }
        }

        // KARAKTER BAZLI ARAMA (5+ karakter veya OEM bulunamadı)
        const b2bPoolMain = await sql.connect(b2bConfig);
        const searchRequest = b2bPoolMain.request();
        // Hem orijinal sorguyu hem normalize edilmiş halini gönderiyoruz,
        // ancak eşleşme kontrolünde normalize edilmiş değer kullanılmalı
        ...
        console.log(`🔍 Karakter bazlı arama: "${normalizedQuery}"`);
        ...
        console.log('🧪 DEBUG smart-search char query:', {
            originalQuery: query,
            normalizedQuery,

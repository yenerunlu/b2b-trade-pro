// /home/yunlu/b2b-app/controllers/b2bAdminController.js - TAM DÜZENLENMİŞ VERSİYON
...
class B2BAdminController {
    ...
    checkAdminAuth(req) {
        try {
            // Basit session kontrolü - localStorage'dan gelen user_data
            ...
            // Kullanıcı tipi kontrolü (1: Admin, 2: Müşteri, 3: Plasiyer)
            const isAdmin = userData.user_type === 'admin' || 
                          userData.user_type === '1' || 
                          userData.user_type === 1;
            
            console.log(`🔐 Auth kontrolü: user_type=${userData.user_type}, isAdmin=${isAdmin}`);
            return isAdmin;
        } catch (error) {
    ...
    clearB2BCache(cacheKey = null) {
        try {
            if (cacheKey) {
                ...
                this.cache.clear();
                console.log(`🧹 Tüm cache temizlendi: ${previousSize} kayıt silindi`);
            }
        } catch (error) {
    ...
    async logAction(logType, message, userCode, ipAddress = '') {
        try {
            ...
            console.log(`📝 Log kaydedildi: ${logType} - ${message}`);
        } catch (error) {
    ...
    async getSystemStats() {
        try {
            ...
            console.error('❌ Sistem istatistikleri hatası:', error.message);
            return {
                activeCampaigns: 0,
                customerOverrides: 0,
                totalSettings: 0,
        ...
        try {
            ...
            try {
                ...
                if (campaign.id) {
                    ...
                    // Yeni ekleme
                    const insertQuery = `
                        INSERT INTO b2b_campaign_items 
                    ...
                    logMessage = `Yeni kampanya eklendi: ${campaign.item_code} (ID: ${newId})`;
                    console.log(`✅ Yeni kampanya eklendi: ${campaign.item_code} (ID: ${newId})`);
                }
        ...
        try {
            ...
            try {
                ...
                if (override.id) {
                    ...
                    console.log(`✅ Müşteri override güncellendi: ${override.customer_code}`);
                    
                } else {
                    // Yeni ekleme
                    const insertQuery = `
                        INSERT INTO b2b_customer_overrides 
